...
namespace Read_FPGA_Data
{
    ...
    partial class FPGA
    {
        ...
        public static bool OpenDevice() //by default open device zero
        {
            ...
            FTDI.FT_STATUS S = USBPort.OpenBySerialNumber("FTVBT12L");
            if (S != FTDI.FT_STATUS.FT_OK)
             {
                Console.WriteLine("Description Not Ok");
                return false;
            }
            ...
            S = USBPort.SetFlowControl(FTDI.FT_FLOW_CONTROL.FT_FLOW_NONE, 0, 0);
            if (S != FTDI.FT_STATUS.FT_OK) 
            {  
                Console.WriteLine("Flow Not Ok");
                return false;
            }
            ...
            if (S != FTDI.FT_STATUS.FT_OK) 
             {
                Console.WriteLine("RTS Not Ok");
                return false;
            }

            S = USBPort.SetDataCharacteristics(FTDI.FT_DATA_BITS.FT_BITS_8, FTDI.FT_STOP_BITS.FT_STOP_BITS_1, FTDI.FT_PARITY.FT_PARITY_NONE);
            if (S != FTDI.FT_STATUS.FT_OK)
             {
                 Console.WriteLine("Characteristics Not Ok");
                return false;
            }
